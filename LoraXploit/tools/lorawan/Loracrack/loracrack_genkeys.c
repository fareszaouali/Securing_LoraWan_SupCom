...
#include <openssl/evp.h>  // Ensure that EVP functions are included
#include <openssl/aes.h>  // Ensure that AES functions are included
...
int main (int argc, char **argv)
{
    ...
    // Cipher vars
    EVP_CIPHER_CTX *ctx_aes128 = EVP_CIPHER_CTX_new();  // Dynamically allocate context
    if (ctx_aes128 == NULL) {
