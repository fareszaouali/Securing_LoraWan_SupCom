...
#include <openssl/evp.h>  // Ensure that the EVP functions are included
#include <openssl/aes.h>  // Ensure that the AES functions are included
...
int main (int argc, char **argv)
{
        ...
        // Cipher vars
        EVP_CIPHER_CTX *ctx_aes128 = EVP_CIPHER_CTX_new();  // Allocate context using EVP_CIPHER_CTX_new()

        if (ctx_aes128 == NULL) {
            error_die("Failed to create EVP_CIPHER_CTX context.");
        }
        ...
        EVP_CIPHER_CTX_free(ctx_aes128);  // Clean up the context
