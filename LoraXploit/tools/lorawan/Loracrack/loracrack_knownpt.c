...
#include <openssl/evp.h>  // Include EVP for AES
...
// Thread for trying to crack certain ranges
void *loracrack_thread(void *vargp)
{
    ...
    EVP_CIPHER_CTX *ctx_aes128_decrypt = EVP_CIPHER_CTX_new();
    ...
    while (AppNonce_current < AppNonce_end && !cracked)
    {
        ...
        while (DevNonce < 0xffff)
        {
            ...
            if (memcmp(decrypt_result, plain_text, plain_text_len) == 0)
            {
                ...
                EVP_CIPHER_CTX_free(ctx_aes128_decrypt);
